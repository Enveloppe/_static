        ...
        .then((doc) => {
          ...
          if (partOfText.startsWith("#")) {
            ...
            if (firstPara && firstPara.tagName === "LI") {
              ...
              toDisplay.innerHTML = firstPara.innerHTML.trim().replaceAll("↩", "").replaceAll("¶", "");
              ...
              const section = doc.querySelector(`div.${partOfText.replace("#", "")}`);
              if (section) {
                articleDOM.appendChild(section);
              }
              toDisplay = articleDOM;
              firstPara = toDisplay;
            } else if (firstPara && firstPara.innerText.replace(partOfText).length === 0) {
